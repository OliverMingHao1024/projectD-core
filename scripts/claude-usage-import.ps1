...
if ([string]::IsNullOrWhiteSpace($LedgerPath)) {
    $LedgerPath = Join-Path $root '.local\usage\claude-ledger.jsonl'
}

$resolvedProjection = Resolve-LocalUsageInput `
    -Root $root -Path $ProjectionPath `
    -Label 'Claude usage projection' -MaximumBytes 32KB
$resolvedAccountRead = Resolve-LocalUsageInput `
    -Root $root -Path $AccountReadPath `
    -Label 'Claude account observation' -MaximumBytes 16KB
...
$projection = Read-LocalUsageJson `
    -Path $resolvedProjection -Label 'Claude usage projection'
...
try {
    ...
    throw 'Claude usage projection does not conform to its canonical schema.'
}

$accountRead = Read-LocalUsageJson `
    -Path $resolvedAccountRead -Label 'Claude account observation'
...
$observation = ConvertTo-ProjectDClaudeAccountObservation `
    -Auth $accountRead.auth -EnvironmentState $accountRead.environmentState
$identityArgs = @{
...
Write-ProjectDUsageIdentityDiagnostic -Identity $identity -ProjectRoot $root `
    | Out-Null
$event = ConvertTo-ProjectDClaudeUsageEvent `
