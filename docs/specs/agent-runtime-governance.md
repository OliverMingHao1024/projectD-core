...
- Status: Runtime Governance v2 security-hardened contract active; 2026-09-04 current-policy Codex and Claude bounded single-host live enforcement revalidation verified. Recovery, live observers, broader tool coverage and cross-host integration remain pending. Existing Phase 3 contracts remain reusable evidence, not a freeze on necessary runtime evolution. **2026-09-06 update**: the Bash/PowerShell classifier in `scripts/lib/RuntimePolicy.psm1` was narrowed to recognize a deterministic read-only allowlist (see "Bash/PowerShell read-only reclassification (2026-09-06)" below), which changes the runtime policy digest. This changes the *deny* path exercised by the 2026-09-04 pilot (`git status --short` no longer denies as `command-execute`; it now normalizes as `local-read`/observe-only), so that specific pilot claim no longer applies to the current policy digest. The allow/deny mechanics for every other capability are unchanged and were not independently re-verified live; treat the 2026-09-04 evidence as historical for the prior digest, and the read-only reclassification as unverified live until a fresh single-host pass records it with the new digest.
...
The hook now supports task-scoped authorization envelopes. Local/network reads remain advisory `observe-only`; an unrelated effect grant never promotes them into verified authorization. Effectful or unclassified capabilities without an envelope fail closed with an enforced pre-effect `deny`. A validated envelope must be unexpired, bound to the exact task/host run and current policy id/version/digest, and contain a matching capability/target/effect grant. Unclassified, expired, mismatched, stale-policy, external/destructive-without-grant, or ungranted effects are denied before the legacy operation intent is written. Contract fixtures are accepted only through an explicit test-only hook switch that is restricted to the system temporary directory. Legacy operation-log classification remains a compatibility projection; when v2 produces a verified allow, the legacy authorization fields also project `explicit-current-task` and `authorized=true`, while advisory reads retain host-policy-pending semantics.

On 2026-09-03, trusted interactive Codex CLI `0.152.1` with `gpt-5.6-luna / medium` on Windows completed a hardened same-session flow for the then-current policy bundle. The host hook was subsequently tightened so verified v2 allow also projects exact task authorization into legacy operation evidence. On 2026-09-04, the updated policy bundle (`sha256:56c7cc9e5b197627681cf6aaae82cefbe6462823de812187681b1c1dbd100107`) was revalidated in a fresh same-session Codex flow: a pinned `codex.list_mcp_resources` bootstrap produced succeeded `network-read / observe-only` evidence; an exact user-authorized task/run envelope allowed one built-in workspace write as `workspace-write / enforced allow`; the legacy operation log projected `explicit-current-task / authorized=true` and closed with matching authorization evidence; and an out-of-scope `git status --short` shell request was denied as `command-execute / capability-not-granted` before effect with no corresponding operation intent. Runtime decisions, authorization and operation evidence remained metadata-only. This verifies the current bounded Codex paths exercised by the pilot. On 2026-09-04, a matching bounded Claude Code single-host flow on the same current policy bundle was also verified: built-in `Read` produced `local-read / observe-only` evidence; an exact user-authorized workspace grant allowed one built-in `Write` with matching legacy `explicit-current-task` evidence; and an out-of-scope `git status --short` Bash request was denied as `command-execute / capability-not-granted` before any operation intent was persisted. Other Codex/Claude models and tool paths, crash/reopen recovery, live observers and cross-host behavior remain unverified. The 2026-09-02 Codex CLI `0.145.0` run remains historical Windows transport evidence for an older policy digest.
...
### Bash/PowerShell read-only reclassification (2026-09-06)

The 2026-09-04 pilot evidence above shows a plain `git status --short` shell request being denied as `command-execute / capability-not-granted` on both Codex and Claude. That was a deliberate fail-safe default at the time, but it overclassified every non-mutating shell invocation (including `git status`, `git log`, `git diff`, `ls`, `cat`, `--version`/`--help` queries) at the same open-world external+destructive severity as an arbitrary, unknown command — broader than the L0 high-risk scope (`core/constitution/rules.md` rule 1: deletion, history rewrite, force push, `reset --hard`, production/database mutation, publish/deploy, external data transmission) and broader than this document's own stated correction ("Refine tool classification from name-level to operation-level... known operations should not stay permanently overclassified simply because a tool name is broad").

`Get-ProjectDRuntimeRequest` in `scripts/lib/RuntimePolicy.psm1` now recognizes a deterministic read-only allowlist before falling through to the repository-mutate/command-execute checks: known read-only git subcommands (`status`, `log`, `diff`, `show`, `blame`, `describe`, `rev-parse`, `ls-files`, `ls-tree`, `cat-file`, `remote -v`, `branch --list`/`-v`, `tag --list`, `stash list`, `config --get`/`--list`), common filesystem read commands (`ls`/`dir`/`Get-ChildItem`, `cat`/`type`/`Get-Content`, `pwd`/`Get-Location`, `head`, `tail`, `wc`, `Select-String`/`grep`/`rg`, `file`, `stat`, `tree`), and any command whose sole trailing argument is `--version`/`-v`/`--help`/`-h`. The command string is split on shell separators (`&&`, `||`, `;`, `|`, newline) and classified as `local-read` (advisory `observe-only`, no authorization required) only when **every** segment matches the allowlist and **no** segment contains redirection (`<`, `>`), backticks, or `$(...)` command substitution; a single non-matching or metacharacter-bearing segment routes the whole command back through the existing repository-mutate/command-execute path unchanged. This is a name/pattern-level allowlist, not a sandboxed guarantee — a tool that lies about its own behavior under a recognized verb is not caught by this classifier, consistent with this document's existing position that host-level classification is not a containment boundary by itself.

This narrows the fail-safe default named above ("Unknown tool classification defaults to high-risk Action | Keep as fallback, narrow scope") for a small set of *known* verbs; unknown or ambiguous commands still fall through to `command-execute` with no change in severity. `scripts/tests/governance-runtime-policy.contract.ps1` adds deterministic contract cases for a bare `git status`, a fully read-only chained command, a bare `--version` query, and two adversarial cases (a read-only prefix followed by a non-allowlisted mutating segment, and a read-only verb combined with output redirection) confirming both directions. This change has deterministic contract-test coverage only; it has not been re-verified through a live single-host pilot pass under its new policy digest, so the 2026-09-04 Codex/Claude live-deny evidence for `git status --short` should be read as historical for the policy digest in effect at that time, not as current-digest coverage.

### Self-reported dry-run/what-if reclassification (2026-09-06) — accepted trust boundary

Following the read-only reclassification above, `Get-ProjectDRuntimeRequest` also recognizes an explicit long-form `--dry-run`, `--dryrun`, `--what-if`, `--whatif`, or `-whatif` token anywhere in a command segment as sufficient for that segment to normalize as `local-read`, independent of which verb it is attached to (so it also overrides an otherwise-`repository-mutate` verb, e.g. `git push --dry-run`).

This is a materially weaker guarantee than the verb allowlist above and was added as an explicit, informed trade-off after the option was presented with its risk: a verb like `git status` is read-only by the tool's own definition regardless of what the caller claims, but `--dry-run`/`--what-if` is the *caller's* claim about an arbitrary tool's behavior. A destructive command that never implements the flag it is bolted onto (or silently ignores an unrecognized option) is not caught by this classifier — `scripts/tests/governance-runtime-policy.contract.ps1`'s `dryRunUnknownToolRequest` case documents this directly: `npm ci --dry-run` normalizes as `local-read` purely because of the token, with no verification that `npm` actually honors it for that invocation. Treat this specific relaxation as advisory convenience for well-behaved tooling, not as a security boundary; it does not change the containment position stated elsewhere in this document ("Host permission prompts are UX controls, not containment").

One collision is deliberately excluded rather than papered over: `git commit -n` means `--no-verify` (skip hooks; the commit still happens), not dry-run, even though `-n` is genuinely `git push`'s and `git clean`'s dry-run shorthand. Only the long-form spellings above are recognized, and bare `-n`/short flags are never treated as a dry-run signal for this reason. `commitNoVerifyRequest` in the contract test asserts `git commit -n -m wip` still requires authorization as `repository-mutate`.

Like the read-only reclassification, this has deterministic contract-test coverage only and has not been exercised in a live single-host pilot pass.

### Literal-content sanitization before classification (2026-09-06)

While applying the dry-run reclassification above, a pre-existing gap surfaced: the `repository-mutate` external/destructive checks (and, in principle, the read-only and mutate-verb checks) ran a substring match against the *entire raw command string*, including text inside quoted arguments and heredoc bodies. A commit whose message text happens to mention another verb as prose (`git commit -m "mentions git push for context"`) was misclassified with that verb's external/destructive properties, even though the actual shell command has no such effect. This is a false-positive direction (more authorization required than necessary), not a security hole, but it is exactly the kind of overclassification this document has already committed to correcting elsewhere.

`Get-ProjectDCommandClassificationText` in `RuntimePolicy.psm1` now strips heredoc bodies (`<<[-]'WORD'` / `<<WORD` through the matching terminator line) and single/double-quoted string contents before any capability, external, or destructive pattern is evaluated; only the shell's own command structure — verbs, flags, and separators outside of literal payload text — participates in classification. This is a best-effort, regex-based approximation of shell quoting (it does not implement a full POSIX/PowerShell tokenizer), so it is safe in the fail-toward-more-authorization direction but not a guarantee against every conceivable quoting edge case. `scripts/tests/governance-runtime-policy.contract.ps1` adds `dryRunQuotedProseRequest` and `dryRunHeredocSanitizationRequest`, confirming a commit mentioning another verb in its message (plain-quoted and heredoc-embedded) no longer reports a spurious external effect. This has deterministic contract-test coverage only and has not been exercised in a live single-host pilot pass.

### Authorization-issuance ergonomics: auto-discovery and capability sets (2026-09-06)

Every prior fix in this session illustrated the same operational cost directly: because the policy digest is content-derived, editing any of the six digest components invalidates every existing authorization envelope immediately, regardless of remaining expiry, and the person doing the editing had to locate the fresh deny decision's file path and issue one `governance-task-authorization.ps1` call per `(capability, target_class)` pair, in a real terminal, every time. None of the invariants that make this safe were the actual source of friction (real terminal, typed confirmation, decision provenance, policy-digest binding, bounded expiry) — the friction was purely procedural: finding a path, and repeating near-identical invocations.

Two ergonomics-only additions address this without touching any of those invariants:

1. **`-DecisionPath` is now optional.** When omitted, the script auto-discovers the most recently written decision file under the host's own `.local/governance/runtime-policy/<host>/` directory and applies every existing validation to it unchanged (schema conformance, `host_run_id`/`task_ref` identity, current policy digest, `host_observable`, not `unclassified-effect`). This only changes *which* already-valid piece of seed evidence is used; it does not relax any check, and it never determines what is granted — that still comes entirely from `-Capability`/`-TargetClass` or `-CapabilitySet`, which the user still explicitly names and confirms.
2. **`-CapabilitySet governance-dev`** grants a fixed, named bundle of `(repository-mutate/governance-control, workspace-write/workspace-file, repository-mutate/repository-state)` — the three pairs this session needed repeatedly while iterating on the runtime policy itself — under a *single* real-terminal confirmation (`AUTHORIZE <host> SET governance-dev`) instead of three. Every pair in a bundle is hard-coded to `allow_external=false, allow_destructive=false`; `-AllowExternal`/`-AllowDestructive` are rejected together with `-CapabilitySet` and remain usable only with an explicit single `-Capability`/`-TargetClass` pair, so `production-mutate` and an external/destructive `command-execute` grant can never come from a bundle.

This does not, and must not, reduce how often re-authorization is needed while actively editing the policy itself — that digest-invalidates-on-edit behavior is the load-bearing property preventing an agent from loosening its own authorization while simultaneously changing the rules that grant it (see "Authorization is task-scoped and non-transitive" above). What changed is how much manual work each *individual* re-authorization costs, not how many times it is required. There is no automated contract-test coverage for this script: it deliberately throws when `[Console]::IsInputRedirected`, which is precisely what a typical non-interactive test harness does, so verification here is manual human trial rather than a `*.contract.ps1` case.
...
The current policy bundle completed bounded single-host revalidation on Codex and Claude on 2026-09-04. Those passes satisfy the pilot gate only for the exercised read/bootstrap, workspace-write allow and out-of-scope command deny paths. The broader Phase 3 host pilot remains open for recovery, observer and coverage items not exercised by those flows.
