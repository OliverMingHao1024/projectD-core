...
description: Turn an unresolved decision into a questionnaire for another person to complete.
---
...
3. **Write the questionnaire.** Draft questions aimed at the gap from steps 1–2, following the Document structure below. Propose the filename `to-questionnaire-<slug>.md` (slug from the topic) and where to write it — the current directory by default — and confirm before writing; the user may prefer a different location or an OS temp path instead of a repo file. Report the path once written. Done when the file exists and every item the user named in step 2 is covered by a question.
