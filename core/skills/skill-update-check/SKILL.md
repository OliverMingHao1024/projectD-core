...
description: Check adopted GitHub-sourced CanonicalSkills for relevant upstream path changes by comparing pinned content digests. Use when the user asks whether imported Skills have updates, wants a read-only upstream drift report, or needs to identify which adopted Skills require re-review.
---
...
Report `unchanged`, `update-available`, or `error`. Do not modify the registry,
